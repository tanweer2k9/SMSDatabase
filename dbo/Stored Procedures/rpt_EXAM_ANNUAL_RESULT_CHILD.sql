...
CREATE PROC [dbo].[rpt_EXAM_ANNUAL_RESULT_CHILD]
@dt_child dbo.[type_EXAM_ANNUAL_REPORT_CHILD_TABLE] readonly,
@dt_parent dbo.[type_EXAM_PROGRESS_REPORT_PARENT_TABLE] readonly

AS


--declare @dt_parent table(
...
--	[Leave] [int] NULL,
--	[Late] [int] NULL,
--	[Max Marks] [float] NULL,
--	[term_id] [int] NULL,
--	[class_id] [int] NULL,
--	[Section] [nvarchar](100) NULL,
--	[Shift] [nvarchar](100) NULL,
--	[Total Pass Marks] [float] NULL,
...
declare @ClassIDForTotalTerms int = 0
select top(1) @ClassIDForTotalTerms = [Class ID] from @dt_child 
set  @term_rank_count = (select COUNT(distinct DEF_TERM) from SCHOOL_PLANE_DEFINITION where DEF_CLASS_ID = @ClassIDForTotalTerms)
...
select @test_total_marks = SUM(TotalMarks), @test_pass_marks = SUM(PassMarks), @test_obtain_marks = SUM(ObtainMarks),@test_percent = SUM(Perent) from @tbl_test_marks
			...
			if @std_id = 30256 and @subject = 'English' and @obtain_marks = 90
			BEGIN
				declare @newasds int = 0
			...
			CAST(
			((CASE WHEN @obtain_marks LIKE '%[0-9]%' THEN CAST(@obtain_marks as float) ELSE CAST(0 as float) END) * (@next_term_age / 100.00 )) + 
			@test_obtain_marks + 
			((CASE WHEN [Obtained Marks] LIKE '%[0-9]%' THEN CAST([Obtained Marks] as float) ELSE CAST(0 as float) END) * ((100.00 - @next_term_age - @test_percent)/ 100.00 )) 
			as nvarchar(50))
			...
			term_id, [Term Rank], [Next Term %age],[Final Term %age],0,CASE WHEN [Obtained Marks] = 'Absent' THEN 'Absent' WHEN [Obtained Marks] = 'Not Taken' THEN 'Not Taken' ELSE '' END, [Marks Type], @class_id,0,0,0,0,''
...
update @dt_annual set [Obtained Marks] = CASE WHEN Grade = 'Absent' THEN 'Ab' WHEN Grade = 'Not Taken' THEN '-' ELSE [Obtained Marks] END, Grade = CASE WHEN Grade = 'Absent' THEN '-' WHEN Grade = 'Not Taken' THEN '-'  ELSE dbo.get_GRADE_NAME([Class ID], 
(CASE WHEN [Percent] LIKE '%[0-9]%' THEN dbo.CSTF([Percent]) ELSE 0 END)) END
where [Total Marks] LIKE '%[0-9]%' and [Obtained Marks] LIKE '%[0-9]%'
