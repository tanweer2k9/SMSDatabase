...
CREATE PROC [dbo].[usp_eLearningGetOfflineLectures]
@UserId numeric
As
...
BEGIN
	select * from
	(select distinct el.Date,sp.Name Class, s.Name Subject, el.Topic, TRIM(el.VideoUrl) VideoUrl,ISNULL(el.GoogleDriveKey,'') GoogleDriveKey,el.Description,el.Id  from SCHOOL_PLANE_DEFINITION spd
	...
	join eLearningOfflineLectures el on el.ClassId = sp.ID and el.SubjectId =  spd.DEF_SUBJECT 
...
BEGIN
	 select distinct el.Date,c.CLASS_NAME Class, s.SUB_NAME Subject, el.Topic, TRIM(el.VideoUrl) VideoUrl,ISNULL(el.GoogleDriveKey,'') GoogleDriveKey,el.Description,el.Id 
 
 from eLearningOfflineLectures el
