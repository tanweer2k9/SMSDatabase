CREATE PROCEDURE [dbo].[sp_STAFF_DEDUCTION_selection_all_staff]
...
select TECH_ID as ID, TECH_EMPLOYEE_CODE as Code,TECH_FIRST_NAME + ' ' + TECH_LAST_NAME as Name, d.DEP_NAME as Department, t.TECH_JOINING_DATE as [Joining Date],
(DATEDIFF(MM,t.TECH_JOINING_DATE,GETDATE()) + 1) as [Duration Months], t.TECH_SALLERY as [Basic Salary], ISNULL(du.STAFF_DEDUCTION_VAL_TYPE, 'Monthly') as [Value Type],
ISNULL(du.STAFF_DEDUCTION_MONTHS, '') as [Months],ISNULL(du.STAFF_DEDUCTION_TYPE, 'Fixed') as [Type],ISNULL(du.STAFF_DEDUCTION_PRCNT_BS, 0) as [Amount],
'T ' [Is Deduction]

from TEACHER_INFO t
join DEPARTMENT_INFO d on t.TECH_DEPARTMENT = d.DEP_ID

left join STAFF_DEDUCTION du on du.STAFF_DEDUCTION_STAFF_ID = t.TECH_ID and du.STAFF_DEDUCTION_DED_ID = @DEDUCTION_ID
