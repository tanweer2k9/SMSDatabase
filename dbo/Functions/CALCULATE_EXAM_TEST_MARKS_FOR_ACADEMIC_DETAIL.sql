...
CREATE FUNCTION [dbo].[CALCULATE_EXAM_TEST_MARKS_FOR_ACADEMIC_DETAIL] (@CLASS_ID numeric, @SUBJECT_ID numeric ,  @TERM_ID numeric,@STD_ID numeric)
...
insert into @t
select ISNULL(SUM(EXAM_TEST_TOTAL_MARKS) ,0) [TotalMarks],ISNULL(SUM(EXAM_ENTRY_OBTAIN_MARKS),0) [ObtainMarks],ISNULL(SUM(EXAM_TEST_TOTAL_MARKS * ([EXAM_TEST_PASS_%AGE] / 100)) ,0) [PassMarks],@PERCENT_Test Perent, @Test [Type] from
...
select ISNULL(SUM(EXAM_TEST_TOTAL_MARKS) ,0) [TotalMarks],ISNULL(SUM(EXAM_ENTRY_OBTAIN_MARKS),0) [ObtainMarks],ISNULL(SUM(EXAM_TEST_TOTAL_MARKS * ([EXAM_TEST_PASS_%AGE] / 100)) ,0) [PassMarks],@PERCENT_Assignment Perent, @Assignment [Type] from
(select ROW_NUMBER() over (order by exam_entry_obtain_marks desc) as sr, * from EXAM_ENTRY e
join EXAM_TEST et on et.EXAM_TEST_ID = e.EXAM_ENTRY_PLAN_EXAM_ID
...
select ISNULL(SUM(EXAM_TEST_TOTAL_MARKS) ,0) [TotalMarks],ISNULL(SUM(EXAM_ENTRY_OBTAIN_MARKS),0) [ObtainMarks],ISNULL(SUM(EXAM_TEST_TOTAL_MARKS * ([EXAM_TEST_PASS_%AGE] / 100)) ,0) [PassMarks],@PERCENT_Presentation Perent, @Presentation [Type] from
(select ROW_NUMBER() over (order by exam_entry_obtain_marks desc) as sr, * from EXAM_ENTRY e
join EXAM_TEST et on et.EXAM_TEST_ID = e.EXAM_ENTRY_PLAN_EXAM_ID
...
select ISNULL(SUM(EXAM_TEST_TOTAL_MARKS) ,0) [TotalMarks],ISNULL(SUM(EXAM_ENTRY_OBTAIN_MARKS),0) [ObtainMarks],ISNULL(SUM(EXAM_TEST_TOTAL_MARKS * ([EXAM_TEST_PASS_%AGE] / 100)) ,0) [PassMarks],@PERCENT_Quiz Perent, @Quiz [Type] from
(select ROW_NUMBER() over (order by exam_entry_obtain_marks desc) as sr, * from EXAM_ENTRY e
join EXAM_TEST et on et.EXAM_TEST_ID = e.EXAM_ENTRY_PLAN_EXAM_ID
